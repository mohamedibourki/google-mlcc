...
# Read dataset - Updates dataframe to use specific columns.
...
print("No" if missing_values == 0 else "Yes")
...
def plot_model(df, features, weights, bias, fig):
    ...
    if len(features) == 1:
        ...
        z = [
            df[z_name].min(),
            (df[z_name].max() - df[z_name].min()) / 2,
            df[z_name].max(),
        ]
        y = [
            df[y_name].min(),
            (df[y_name].max() - df[y_name].min()) / 2,
            df[y_name].max(),
        ]
        x = []
        ...
        model = go.Figure(
            data=go.Surface(
                x=plane["x"], y=plane["y"], z=plane["z"], colorscale=light_yellow
            )
        )
...
def run_experiment(df, feature_names, label_name, learning_rate, epochs, batch_size):
    print(
        "INFO: starting training experiment with features={} and label={}\n".format(
